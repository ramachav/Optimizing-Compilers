...
void create_leaf(string name_leaf, string type_leaf, string string_leaf, float float_leaf, int int_leaf) {
  ...
  if(type_leaf == "Read_Write") {
    ...
    if(check_leaf(name_leaf, current_leaf_scope) != NULL) {
      ...
      current_leaf = new symbol_table_tree;
...
void start_block_scope(string name) {
  ...
  new_leaf = new symbol_table_tree;
  new_leaf->leaf_name = name;
...
void print_symbol_table_tree(symbol_table_tree *trunk) {
  ...
  while(temp_stem != NULL) {
	...
	cout<<"Symbol table "<<temp_stem->leaf_scope<<" "<< ++block_count<<"\n";
      else
	cout<<"Symbol table "<<temp_stem->leaf_scope<<"\n";
    }
    else {
...
void set_register_Rd(threeAC_node * temp_node) {
  ...
  temp_node->reg_dest = "$T";
  temp_node->reg_dest += int2string;
}
...
void clean_3ac_list(list_data ptr) {
  ...
  for(index = threeAC_list.begin(); index != threeAC_list.end(); index++) {
    if((*index)->Rs == "+" || (*index)->Rs == "-" || (*index)->Rs == "*" || (*index)->Rs == "/") {
      --index;
      string temp_value = (*index)->reg_dest;
      ++index;
      (*index)->Rs = temp_value;
    }
    if((*index)->Rt == "+" || (*index)->Rt == "-" || (*index)->Rt == "*" || (*index)->Rt == "/") {
      --index;
      --index;
      string temp_value = (*index)->reg_dest;
      ++index;
